...
export default function MyComponent({ props01, props02, props03, props04, props05, props06, props07, props08, props09 }) {
    return (
        <Fragment>
            ...
            <span>props05: { props05 ? props05.map((e, i) => <b key={i}>{e}{' '}</b>) : '---not set---' }</span><br/>
            <span>props06: { props06 ? props06() : '---not set---' }</span><br/>
            <span>props07: { props07 ? props07 : '---not set---' }</span><br/>
            <span>props08: { props08 ? props08.map((e, i) => e ? <b key={i}>{'true'}{' '}</b> : <b key={i}>{'false'}{' '}</b>) : '---not set---' }</span><br/>
            <span>
                props09: 
                    { 
                        props09 ? 
...
MyComponent.propTypes = {
    ...
    props02: PropTypes.number.isRequired, // 경고 표시
